...
#define		BOOT_WAIT_TIME				(500)	// The time in milliseconds to wait at startup.
#define		INIT_WAIT_TIME				(2)		// Initial wait time between module probes.
...
void xmitWait(void);
// Wait before transmitting in the case of repetitive transmissions.
void preXmitWait(int currentState);
// Function that waits for BOOT_WAIT_TIME milliseconds.
void bootWait(void);
...
{	
	...
	// Unload all of the configurations.
	unloadAllConfigs();
	
	// Wait for every module to boot.
	bootWait();
...
void initializeChildren(void)
{
	...
	while(CHILD == 0)
	{
		...
		if(!childListen())
		{
			// Wait for INIT_WAIT_TIME before we try again.
